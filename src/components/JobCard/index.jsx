      ...
      <div className="flex justify-between items-center mx-10 mb-4 py-6 px-10 bg-zinc-900 rounded-md text-white shadow-lg border border-zinc-900 hover:translate-y-1 hover:scale-104">
        <div className='flex flex-col items-start gap-3'>
          ...
          <div className="flex items-center gap-2">
            {props.skills.map((skill, i)=>(<p key={i} className='text-zinc-300 py-1 px-2 rounded-md border border-white'>{skill}</p>))}
          </div>
        </div>
        <div className="flex items-center gap-4">
          <p className="text-zinc-400">Posted {diffInDays > 1? `${diffInDays} days`:`${diffInDays} day`} ago</p>
